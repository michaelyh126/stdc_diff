...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Batch visualize three STDC checkpoints and GT differences.'
    )
    parser.add_argument(
        '--config',
        required=True,
    ...
    parser.add_argument(
        '--label-dir',
        required=True,
        help='directory containing binary ground-truth labels')
    parser.add_argument(
...
def find_label_path(img_path, img_dir, label_dir, label_exts):
    rel_path = os.path.relpath(img_path, img_dir)
    rel_root, rel_ext = os.path.splitext(rel_path)
    stem = os.path.splitext(os.path.basename(img_path))[0]

    candidates = [os.path.join(label_dir, rel_path)]
...
def safe_output_name(img_path, img_dir):
    ...
    return re.sub(r'[^0-9A-Za-z_.-]+', '_', rel_root.replace(os.sep, '_'))
...
def process_one_image(img_path, label_path, image_out_dir, models, args):
    image_rgb = read_image_rgb(img_path)
    gt = read_groundtruth(label_path)
...
def main():
    ...
    image_paths = find_image_files(args.img_dir, args.image_exts,
                                   args.recursive)
    ...
    for index, img_path in enumerate(image_paths, start=1):
        label_path = find_label_path(img_path, args.img_dir, args.label_dir,
                                     args.label_exts)
        rel_img_path = os.path.relpath(img_path, args.img_dir)
        if label_path is None:
            missing_labels.append(rel_img_path)
            if args.skip_missing_label:
                print(f'[{index}/{len(image_paths)}] skip missing label: '
                      f'{rel_img_path}')
                continue
            raise FileNotFoundError(
                f'Cannot find label for image: {img_path}')

        rel_label_path = os.path.relpath(label_path, args.label_dir)
        output_name = safe_output_name(img_path, args.img_dir)
        image_out_dir = os.path.join(per_image_root, output_name)
        print(f'[{index}/{len(image_paths)}] processing {rel_img_path}')
