...
class DualDistill(EncoderDecoderPid):
    ...
    def __init__(self,
        ...
        super(DualDistill, self).__init__(
            backbone=backbone,
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        img_refine=img
        losses = dict()
        feats,loss_decode = self._decode_head_forward_train(x, img_refine, img_metas, gt_semantic_seg)
        losses.update(loss_decode)

        # feat_=[]
        # feat_.append(feats[0])
        # feat_in=[]
        # feat1 = F.interpolate(
        #     feats[1],
        #     size=[feats[4].shape[-2],feats[4].shape[-1]],
        #     mode='bilinear', align_corners=False)
        # x_out=torch.cat((feat1,feats[4]),dim=1)
        # feat_in.append(x_out)
        # feat_in.append(feats[3])
        # feat_in.append(feats[2])
        # feat_.append(feat_in)
        # loss_distill,loss_teacher=self.distill_head.forward_train(feat_,gt_semantic_seg=gt_semantic_seg)
        # losses.update(loss_distill)
        # losses.update(loss_teacher)
    ...
    def _decode_head_forward_train(self, x, img, img_metas, gt_semantic_seg):
        ...
        losses = dict()
        prev_features=None
        feats,loss_refine, *loss_contrsative_list = self.refine_head.forward_train(img, prev_features, img_metas, gt_semantic_seg, self.train_cfg)
