...
class IsdHarrDiff(EncoderDecoder):
    ...
    def __init__(self,
        ...
        super(IsdHarrDiff, self).__init__(
            backbone=backbone,
    ...
    def encode_decode(self, img, img_metas):
        ...
        # mask_diff=torch.sigmoid(diff_pred)
        # mask_diff = (mask_diff > 0.5).float()
        # mask_deep=torch.argmax(out_g, dim=1).unsqueeze(1)
        out = self.refine_head.forward_test(img_refine, prev_outputs, img_metas, self.test_cfg, diff_pred)
    ...
    def _decode_head_forward_train(self, x, img, img_metas, gt_semantic_seg):
        ...
        loss_refine, *loss_contrsative_list = self.refine_head.forward_train(img, prev_features, img_metas, gt_semantic_seg, self.train_cfg,diff_pred)
