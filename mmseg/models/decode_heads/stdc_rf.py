...
class ShallowNet_rf63(nn.Module):
    def __init__(self, base=64, in_channels=3, layers=[2,2], block_num=4,
                 type="cat", dropout=0.20,
                 pretrain_model="D:\\isd\\pretrained models\\STDCNet813M_73.91.tar",
                 num_classes=2):
        ...
        self.in_channels = in_channels
        self.cls_feat8 = ConvX(base * 4, num_classes, 3, 1)
        self.cls_feat8_deep = ConvX(base * 8, num_classes, 3, 1)

        self.features = self._make_layers_rf63(base, layers, block_num, block)
        ...
        # 第二个 stage：不再下采样，继续 stride=1 细化
        self.x8_deep = nn.Sequential(self.features[4:6])

        # 不加载预训练，先直接初始化
        # 因为结构已经和原 STDC 不一致，直接对齐预训练会比较麻烦
        self.init_params()

        # 如果你后面想部分加载预训练，可以再单独改
        # if pretrain_model:
        #     print('use pretrain model {}'.format(pretrain_model))
        #     self.init_weight(pretrain_model)
        # else:
    ...
    def _make_layers_rf63(self, base, layers, block_num, block):
        """
        目标：
        - stem: x2, x4
        - 第一个 stage: 正常下采样到 x8
        - 第二个 stage: 不再下采样，只做 stride=1 细化
        """
        features = []

        # stem
        features += [ConvX(self.in_channels, base // 2, 3, 2)]   # x2
        features += [ConvX(base // 2, base, 3, 2)]               # x4

        # -------------------------
        # stage 1 -> x8
        # -------------------------
        # 第一个 block 仍然 stride=2
        features.append(block(base, base * 4, block_num, 2))
        ...
        # stage 2 -> 仍然保持 x8
        # -------------------------
        # 原来这里 j==0 会 stride=2 到 x16
        # 现在改成 stride=1，只升通道，不降采样
        features.append(block(base * 4, base * 8, block_num, 1))
        features.append(block(base * 8, base * 8, block_num, 1))

        return nn.Sequential(*features)

    def forward(self, x, cas3=False, return_cls=False):
        feat2 = self.x2(x)
        feat4 = self.x4(feat2)
        ...
        # 后续仍然保持 x8，不再下采样
        feat8_deep = self.x8_deep(feat8)

        if return_cls:
            feat8_cls = self.cls_feat8(feat8)
            feat8_deep_cls = self.cls_feat8_deep(feat8_deep)
            return feat8_cls, feat8_deep_cls

        if cas3:
            return feat4, feat8, feat8_deep
        else:
            return feat8, feat8_deep
