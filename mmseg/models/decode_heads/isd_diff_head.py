...
class ISDDiffHead(BaseCascadeDecodeHead):
    def __init__(self, down_ratio, prev_channels, reduce=False, **kwargs):
        super(ISDDiffHead, self).__init__(**kwargs)
        ...
        self.diff_fusion1=FeatureFusionModule(256,128)
        self.diff_fusion2=FeatureFusionModule(256,128)
        self.harr_up=HarrUp()
    ...
    def forward(self, inputs, prev_output,  train_flag=True, mask=None):
        """Forward function."""
        ...
        if self.reduce is not None:
            deep_feat = self.reduce(deep_feat_up)
        # stage 1
        _, aux_feat16, fused_feat_16 = self.fuse16(shallow_feat16, deep_feat_up)
        # stage 2
        _, aux_feat8, fused_feat_8 = self.fuse8(shallow_feat8, fused_feat_16)
        # # stage 1
        # _, aux_feat16, fused_feat_16 = self.diff_fusion1(shallow_feat16, deep_feat,mask)
        # # stage 2
        # _, aux_feat8, fused_feat_8 = self.fuse8(shallow_feat8, fused_feat_16)
    ...
    def forward_train(self, inputs, prev_output, img_metas, gt_semantic_seg, train_cfg,mask=None):
        seg_logits, seg_logits_aux16, seg_logits_aux8, losses_recon, losses_fa = self.forward(inputs, prev_output,mask=mask)
    ...
    def forward_test(self, inputs, prev_output, img_metas, test_cfg,mask):
        ...
        return self.forward(inputs, prev_output, False,mask=mask)
