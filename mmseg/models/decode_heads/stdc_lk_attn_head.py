...
class STDCStageAttention(nn.Module):
    def __init__(self, num_branches, kernel_size=7):
        super(STDCStageAttention, self).__init__()
        self.conv = nn.Conv2d(
            2,
            num_branches,
            kernel_size,
...
class CatBottleneck(nn.Module):
    def __init__(self, in_planes, out_planes, block_num=3, stride=1):
        ...
        assert block_num > 1, print("block number should be larger than 1.")
        self.conv_list = nn.ModuleList()
        self.stage_attn = STDCStageAttention(block_num)
        self.stride = stride
        if stride == 2:
            self.avd_layer = nn.Sequential(
                nn.Conv2d(out_planes//2, out_planes//2, kernel_size=3, stride=2, padding=1, groups=out_planes//2, bias=False),
    ...
    def forward(self, x):
        ...
        out = self.stage_attn(out_list)
        return out

class ShallowNet_lk_attn(nn.Module):
    def __init__(self, base=64, in_channels=3, layers=[2,2], block_num=4, type="cat", dropout=0.20, num_classes=2):
        super(ShallowNet_lk_attn, self).__init__()
        if type == "cat":
            block = CatBottleneck
        elif type == "add":
            block = AddBottleneck
...
class ShallowNet_lk2_attn(nn.Module):
    def __init__(self, base=64, in_channels=3, layers=[2,2], block_num=4, type="cat", dropout=0.20, num_classes=2):
        super(ShallowNet_lk2_attn, self).__init__()
        if type == "cat":
            block = CatBottleneck
        elif type == "add":
            block = AddBottleneck
