...
class VitGuidanceHead(BaseDecodeHead):
    ...
    def forward(self, inputs,decoder_feature=None,decoder_seg_logits=None):
        ...
        if self.inter_transform_type == 'linear':
            ...
            x_guidance_feature[1][1]= F.interpolate(
                                        x_guidance_feature[1][1],
                                        size=[feat_t[2].shape[-2], feat_t[2].shape[-1]],
                                        mode='bilinear', align_corners=False)
            x_guidance_feature[1][2]= F.interpolate(
                                        x_guidance_feature[1][2],
                                        size=[feat_t[3].shape[-2], feat_t[3].shape[-1]],
                                        mode='bilinear', align_corners=False)
    ...
    def forward_train(self, inputs,decoder_feature=None,decoder_seg_logits=None,gt_semantic_seg=None, *args):
        ...
        predict= seg_logits[0][0]
        predict = F.interpolate(predict, gt_semantic_seg.size()[2:], mode='bilinear', align_corners=False)
        losses_teacher=self.ohem(predict, gt_semantic_seg.squeeze(1), 'teacher_')
        return losses,losses_teacher
