...
# from .pidnet_harr import PIDNet
...
class OhemCrossEntropy(nn.Module):
    ...
    def forward(self, score, target):
        ...
        if len(balance_weights) == len(score):
            ...
            loss['ohem_loss']=sb_weights * self._ohem_forward(score[0], target)
            return loss
...
class STDCInHead(BaseCascadeDecodeHead):
    def __init__(self, down_ratio, prev_channels,img_size, reduce=False, **kwargs):
        super(STDCInHead, self).__init__(**kwargs)
        ...
        #                             channels=self.channels, up_lists=[4, 4, 2])
        # self.compress=nn.Conv2d(512,self.channels,kernel_size=1,stride=1)
        # self.pointrend=PointHead(in_channels=self.num_classes,channels=self.num_classes,num_classes=self.num_classes)
    ...
    def forward(self, inputs, prev_output,  train_flag=True, mask=None,gt=None,img_metas=None,train_cfg=None,diff_pred_deep=None):
        ...
        if train_flag:
            ...
            # deep_feat=self.compress(deep_feat)
    ...
    def forward_train(self, inputs, prev_output, img_metas, gt_semantic_seg, train_cfg,mask=None):
        ...
        losses=self.ohem(output,gt_semantic_seg.squeeze(1))
        losses_p = self.ohem(x_extra_p, gt_semantic_seg.squeeze(1))
