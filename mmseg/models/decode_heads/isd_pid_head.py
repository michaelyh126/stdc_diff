...
class ISDPidHead(BaseCascadeDecodeHead):
    def __init__(self, down_ratio, prev_channels, reduce=False, **kwargs):
        super(ISDPidHead, self).__init__(**kwargs)
        ...
        self.pid = PIDNet(m=2, n=3, num_classes=self.num_classes, planes=32, ppm_planes=96, head_planes=128,
                          augment=True)
    ...
    def forward(self, inputs, prev_output, train_flag=True):
        ...
        x_extra_p, x_,x_features = self.pid.forward(high_residual_input)
        deep_feat = prev_output[0]
        if self.reduce is not None:
            deep_feat = self.reduce(deep_feat)
        # stage 1
        _, aux_feat16, fused_feat_16 = self.fuse16(x_features, deep_feat)
        # stage 2
        _, aux_feat8, fused_feat_8 = self.fuse8(x_, fused_feat_16)
        ...
        if train_flag:
            ...
            return output,output_aux16,output_aux8,  losses_re, losses_fa, x_extra_p
        else:
            return output
    ...
    def forward_train(self, inputs, prev_output, img_metas, gt_semantic_seg, train_cfg):
        seg_logits,seg_logits_aux16, seg_logits_aux8,  losses_recon, losses_fa ,x_extra_p= self.forward(inputs, prev_output)
        ...
        return losses,losses_aux16, losses_aux8, losses_recon, losses_fa,losses_p
