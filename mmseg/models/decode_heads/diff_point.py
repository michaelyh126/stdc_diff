...
class DiffPoint(BaseCascadeDecodeHead):
    ...
    def __init__(self,
        ...
        fc_in_channels = self.num_classes
    ...
    def get_points_index(self, mask):
        num_points = mask.sum(dim=(1,2, 3))
        # point_indices = mask.topk(num_points, dim=1)[1]
        point_indices = []
        for i in range(mask.shape[0]):
            # 对每个批次的 mask 应用 topk
            indices = mask[i, 0].topk(num_points[i], dim=0)[1]  # 仅选择通道 0
            topk_indices.append(indices)

        # 转换为张量
        point_indices = torch.stack(topk_indices)  # 形状为 (b, num_points)
    ...
    def get_points(self, point_indices, points, prev_output):
        coarse_point_feats = self._get_coarse_point_feats(
            prev_output, points)
        return coarse_point_feats

    def forward_refine(self, point_indices, points,coarse_point_feats, refined_seg_logits):
        point_logits = self.forward(coarse_point_feats)
        point_indices = point_indices.unsqueeze(1).expand(-1, channels, -1)
        refined_seg_logits = refined_seg_logits.reshape(
            batch_size, channels, height * width)
        refined_seg_logits = refined_seg_logits.scatter_(
